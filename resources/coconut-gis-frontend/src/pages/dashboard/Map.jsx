...
const MapComponent = () => {
	...
	const [selectedAffectedLandsPerBarangay, setSelectedAffectedLandsPerBarangay] =
		useState([]);
	const [
		...
		if (!selectedBarangay) {
			...
			select.on("select", (e) => {
				...
				if (selectedFeature) {
					...
					setPopoverContent(`Farmers: ${farmersCount}\nLands: ${landsCount}`);
					popoverRef.current.style.display = "block";
					map.getOverlayById(popoverRef.current).setPosition(coordinates);
				} else {
	...
	const handleBarangayChange = (e) => {
		...
		if (selectedBarangay) {
			...
			if (selectedBarangay.lands.length > 0) {
				const affected = selectedBarangay.lands.filter((land) => land.is_affected);
		...
		try {
			const response = await fetch(build("farmer/create"), {
				...
				body: JSON.stringify({
					...
					number_of_coconut_trees: Number(details.number_of_coconut_trees),
					...
					<>
							...
							<div className='input-group mb-3 '>
								...
								{showDropdown && filteredFarmers.length === 0 && (
									<div
										...
										<p className='dropdown-item text-muted'>No farmers found</p>
									</div>
								)}
							</div>
						)}
						...
						{selectedBarangay && (
							<>
								<Col className='gap-3 my-4 border-top border-bottom py-3'>
									<Col
										...
										<div className='d-flex flex-row gap-4 justify-items-center'>
											<p className='mb-1 text-muted text-truncate'>Total Farmers</p>
								...
								<div className='mb-3'>
									<label className='form-label'>Full name (First Name, Last Name)</label>
									<input
										type='text'
										className='form-control'
										value={details.name ?? ""}
								...
								<div className='mb-3'>
									<label className='form-label'>Barangay, Municipality</label>
									<input
										type='text'
										className='form-control'
										value={selectedBarangay}
								...
								<div className='mb-3'>
									...
									<input
										type='number'
										className='form-control'
										placeholder={details.number_of_coconut_trees ?? 0}
										onChange={(e) => {
											setDetails({
												...details,
												number_of_coconut_trees: e.target.value,
											});
										}}
									/>
								</div>
								{admin.role === "admin" ? (
									<div className='mb-3'>
										<label className='form-label'>
											Plot Areas that has been un/affected by Pest Infestation
										</label>
										<div className='d-flex gap-2'>
											<button
												onClick={() => {
													toast.success("Unaffected Plot Active. Please plot now!");
													addInteraction("rgba(0, 255, 0, 0.5)", setUnaffectedPlots);
												}}
											...
											<button
												onClick={() => {
													toast.error("Affected Plot Active. Please plot now!");
													addInteraction("rgba(255, 0, 0, 0.5)", setAffectedPlots);
												}}
									...
									<div className='mt-2'>
										<p className='text-center'>Land areas affected by Pest Infestation</p>
										<Row className='gap-2'>
											<Col className='d-flex align-items-center'>
												...
												<p className='mb-0 ms-2'>Affected</p>
											</Col>
											<Col className='d-flex align-items-center'>
												<div
													style={{
												...
												<p className='mb-0 ms-2'>Unaffected</p>
											</Col>
										</Row>
									</div>
								)}
