...
const Events = () => {
	...
	const handleFormSubmit = async (e) => {
		e.preventDefault();
		const { title, start, end, description, location, eligibility } = newEvent;

		if (!title || !start || !end || !description || !location || !eligibility) {
	...
	const handleDeleteEvent = async () => {
		if (idToDelete || typeof idToDelete === "number") {
			try {
				const response = await fetch(build(`event/delete/${idToDelete}`), {
					method: "DELETE",
					headers: {
						"Content-Type": "application/json",
						Authorization: `Bearer ${localStorage.getItem("token")}`,
				...
				<Row style={styles.mainRow}>
								...
								events.map((event, index) => (
									<div
										...
										<h4 style={styles.eventTitle}>
											{event.event_name}
											{admin.role === "admin" && admin.admin_id === event.admin_id && (
										...
										<p style={styles.eventDescription}>{event.event_description}</p>
										<div style={styles.eventDetails}>
											...
											<div style={styles.eventInfo}>
												<FaUserCheck
													...
													style={styles.icon}
												/>
												<span>{event.participant_eligibility}</span>
											</div>
											<div style={styles.eventInfo}>
												<FaCalendarAlt
													size={16}
					...
					<Col
						...
						<Calendar
							...
							events={events}
							components={{
								event: ({ event }) => <span>{event.event_name}</span>,
							}}
